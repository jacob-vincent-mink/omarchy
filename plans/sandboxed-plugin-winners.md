...
The native activation surface (`shell/services/native/SandboxedPluginSurface.qml`) is one nonexclusive overlay-layer canvas on the largest logical screen. Shared widgets now receive real host-owned bar slots, including in a trusted replacement bar. The shared worker now provides own-service/overlay lifecycle and initial/live detached settings and style tokens without restarting plugin instances. The GitHub and Omagotchi review forks now declare sandbox requests; the other private experiments add declarations to staged copies. Generic tests verify slot resizing, all four bar edges, popup anchoring, pointer handoff, movement, hiding and replacement-bar switching. Shared tooltips now preserve the host styling and hover delay, wrap within the private output and pass four-edge hover/dismissal and input-transparency checks. Keyboard panel switching from inside a worker, full font-asset parity and original-plugin desktop acceptance remain required. Host QObject access must not cross the isolation boundary.
