...
- A registered fake-service request without a trusted gesture and a storage request expanding the granted quota are independently denied, durably audited with `gesture_missing` and `outside_scope`, and produce no provider effect.
- An undeclared raw broker type is protocol-fatal before any provider effect. The custom QML `open_uri` action is deliberately unsupported by schema v2: it emits no broker packet, audit record, or host effect and peer exit tears the channel down. A future URL capability must be separately registered, scheme/host scoped, gesture-bound, and reviewed across B2/C4/C8; E3 does not silently manufacture one.
