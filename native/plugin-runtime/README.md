...
Schema v2 is therefore off for users even though its complete reference implementation is built and tested. Discovery and revision APIs default their feature state to disabled, the permission CLI additionally requires trusted rollout state through `OMARCHY_PLUGIN_SCHEMA_V2_ENABLED=1`, and the existing schema-v1 commands remain explicitly unsafe compatibility behavior.
