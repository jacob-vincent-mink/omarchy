...
bool encode_slot_header(std::span<std::byte> destination,
  ...
  std::memcpy(destination.data() + kSequenceOffset, &sequence,
              sizeof(sequence));
  put<std::uint64_t>(destination, kFrameSequenceOffset, frame_sequence);
  put<std::uint64_t>(destination, kSurfaceIdOffset, allocation.surface.id);
  put<std::uint64_t>(destination, kSurfaceGenerationOffset, allocation.surface.generation);
  put<std::uint32_t>(destination, kProfileVersionOffset, kSoftwareProfileVersion);
  put<std::uint32_t>(destination, kHeaderSizeOffset,
        static_cast<std::uint32_t>(kSlotHeaderSize));
  put<std::uint32_t>(destination, kLogicalWidthOffset, allocation.logical_width);
  put<std::uint32_t>(destination, kLogicalHeightOffset, allocation.logical_height);
  put<std::uint32_t>(destination, kPixelWidthOffset, allocation.pixel_width);
  put<std::uint32_t>(destination, kPixelHeightOffset, allocation.pixel_height);
  put<std::uint32_t>(destination, kDprNumeratorOffset, allocation.dpr_numerator);
  put<std::uint32_t>(destination, kDprDenominatorOffset, allocation.dpr_denominator);
  put<std::uint32_t>(destination, kStrideOffset, allocation.stride);
  put<std::uint32_t>(destination, kPixelFormatOffset, allocation.pixel_format);
  put<std::uint32_t>(destination, kDamageCountOffset, 0);
  put<std::uint32_t>(destination, kReservedOffset, 0);
  put<std::uint64_t>(destination, kPayloadLengthOffset, allocation.frame_bytes);
  return true;
}
...
decode_slot_header(std::span<const std::byte> source) {
  ...
  if (get<std::uint32_t>(source, kProfileVersionOffset) != kSoftwareProfileVersion ||
      get<std::uint32_t>(source, kHeaderSizeOffset) != kSlotHeaderSize ||
      get<std::uint32_t>(source, kReservedOffset) != 0 ||
      std::ranges::any_of(
  ...
  TrustedAllocation allocation{
      .surface = {.id = get<std::uint64_t>(source, kSurfaceIdOffset),
                  .generation = get<std::uint64_t>(source, kSurfaceGenerationOffset)},
      .logical_width = get<std::uint32_t>(source, kLogicalWidthOffset),
      .logical_height = get<std::uint32_t>(source, kLogicalHeightOffset),
      .pixel_width = get<std::uint32_t>(source, kPixelWidthOffset),
      .pixel_height = get<std::uint32_t>(source, kPixelHeightOffset),
      .dpr_numerator = get<std::uint32_t>(source, kDprNumeratorOffset),
      .dpr_denominator = get<std::uint32_t>(source, kDprDenominatorOffset),
      .stride = get<std::uint32_t>(source, kStrideOffset),
      .pixel_format = get<std::uint32_t>(source, kPixelFormatOffset),
      .frame_bytes = get<std::uint64_t>(source, kPayloadLengthOffset),
      .page_size = 0,
      .slot_extent = 0,
      .mapping_bytes = 0,
  };
  return DecodedSlotHeader{.sequence = sequence,
                           .frame_sequence =
                               get<std::uint64_t>(source, kFrameSequenceOffset),
                           .allocation = allocation,
                           .damage_count = get<std::uint32_t>(source, kDamageCountOffset)};
}
