...
#include "omarchy/plugin_runtime/fixed_layout.hpp"
...
enum class CommonMessageType : std::uint16_t {
  ...
  typed_error = 0x0004,
  cancel = 0x0005, // Reserved; selected endpoints reject generic cancellation.
...
// The wire order is declared once. FixedLayout generates both directions;
// the independent golden fixtures pin compatibility to the published bytes.
using EnvelopeLayout = omarchy::plugin_runtime::FixedLayout<EnvelopeHeader,
    &EnvelopeHeader::magic, &EnvelopeHeader::envelope_version,
    &EnvelopeHeader::header_size, &EnvelopeHeader::endpoint_role,
    &EnvelopeHeader::message_type, &EnvelopeHeader::role_protocol_version,
    &EnvelopeHeader::flags, &EnvelopeHeader::payload_length,
    &EnvelopeHeader::reserved, &EnvelopeHeader::launch_generation,
    &EnvelopeHeader::correlation_id, &EnvelopeHeader::lane_sequence>;
static_assert(EnvelopeLayout::size == kHeaderSize);
