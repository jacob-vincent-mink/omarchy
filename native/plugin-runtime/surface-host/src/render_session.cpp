...
bool HostRenderSession::receive(const AuthenticatedRenderPacket &incoming) {
  if (!endpoint_ || phase_ == Phase::failed || phase_ == Phase::disconnected ||
      incoming.payload.size() > wire::payload_cap(wire::EndpointRole::render))
  ...
  const wire::PacketView packet{
      .header = {.endpoint_role = wire::EndpointRole::render,
                 ...
                 .flags = 0,
                 .payload_length =
                     static_cast<std::uint32_t>(incoming.payload.size()),
                 .launch_generation = generation_,
