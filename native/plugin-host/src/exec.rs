...
impl Request {
  ...
  pub(crate) fn start(
    ...
    let grant = grants
      .get(&self.name)
      .ok_or_else(|| invalid("host executable was not granted"))?;
    Job::start(
      &grant.executable,
      &grant.tree,
      &grant.selected,
...
fn decode_response(mut packet: Packet) -> io::Result<Output> {
  ...
  struct Rejected {
    version: u32,
    status: crate::notification::Status,
  }
  if packet.fds.is_empty()
    && let Ok(rejected) = serde_json::from_slice::<Rejected>(&packet.bytes)
    && rejected.version == 1
  {
    return Err(io::Error::new(
      if matches!(
        rejected.status,
        crate::notification::Status::Busy | crate::notification::Status::RateLimited
      ) {
        io::ErrorKind::WouldBlock
      } else {
        io::ErrorKind::PermissionDenied
      },
      format!("host execution rejected: {:?}", rejected.status),
    ));
  }
  if packet.bytes != TAG || packet.fds.len() != 1 {
    return Err(invalid("host execution denied or failed"));
  }
  let bytes = payload::read(packet.fds.remove(0), MAX_RESPONSE)?;
  if bytes.len() < 12 {
    return Err(invalid("truncated exec response"));
  }
  ...
  if out > MAX_OUTPUT || err > MAX_OUTPUT || bytes.len() != 12 + out + err {
    return Err(invalid("invalid exec output lengths"));
  }
  Ok(Output {
...
pub fn request(name: String, argv: Vec<String>) -> io::Result<()> {
  let request = Request { name, argv };
  let deadline = Instant::now() + Duration::from_secs(120);
  let backoff = Duration::from_millis(500 + u64::from(std::process::id() % 251));
  let output = loop {
    let channel = match Channel::connect(Path::new("/run/plugin/exec")) {
      Ok(channel) => channel,
      Err(e) if e.kind() == io::ErrorKind::WouldBlock && Instant::now() < deadline => {
...
mod tests {
  ...
  fn exec_transport_requires_sealed_bounded_records_and_preserves_binary_output() {
    ...
    for status in [
      crate::notification::Status::Busy,
      crate::notification::Status::RateLimited,
    ] {
      crate::notification::reply(&a, status).unwrap();
      assert_eq!(receive(&b).unwrap_err().kind(), io::ErrorKind::WouldBlock);
    }
    crate::notification::reply(&a, crate::notification::Status::Failed).unwrap();
