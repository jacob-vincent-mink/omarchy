...
impl Request {
  ...
  pub(crate) fn spawn(
    ...
    let scope = grants
      .get(&self.scope)
      .ok_or_else(|| invalid("HTTP scope was not granted"))?;
    scope.check(self)?;
...
pub fn execute() -> io::Result<()> {
  ...
  let mut outgoing = ureq::http::Request::builder()
    ...
    .uri(url.as_str())
    .header("User-Agent", "omarchy-plugin-host")
...
pub fn receive(channel: &Channel) -> io::Result<(ResponseHead, Vec<u8>)> {
  ...
  loop {
    match channel.receive() {
      ...
      Ok(_) => return Err(invalid("HTTP request denied, busy, rate-limited or failed")),
...
/// Worker CLI: one JSON request on stdin; raw body on stdout. Optional metadata
/// is written only to the worker-selected local file, never a host pathname.
pub fn request(metadata: Option<&Path>) -> io::Result<()> {
  let mut input = Vec::new();
  io::stdin()
  ...
  let channel = Channel::connect(Path::new("/run/plugin/http"))?;
  request.send(&channel)?;
  let (head, body) = receive(&channel)?;
  if let Some(path) = metadata {
    std::fs::write(path, serde_json::to_vec(&head)?)?;
  }
  io::stdout().write_all(&body)?;
  if head.status >= 400 {
    return Err(io::Error::other(format!("HTTP status {}", head.status)));
  }
  Ok(())
}
...
mod tests {
  ...
  fn scope() -> Scope {
    serde_json::from_value(json!({
      "origin": "https://api.github.com", "method": "GET", "path": "/repos/*/*/actions/runs",
      "query": {
  ...
  fn methods_origins_paths_and_required_query_filters_cannot_be_widened() {
    let good = "https://api.github.com/repos/openai/codex/actions/runs?status=in_progress&page=2";
    let selected = scope();
    ...
    selected
      ...
      .unwrap();
    for bad in [
      good.replace("api.github.com", "api.github.com.evil.test"),
      good.replace("api.github.com", "api.github.com:444"),
      good.replace("api.github.com", "user@api.github.com"),
      good.replace("https:", "http:"),
      good.replace("/actions/runs", "/contents/secret"),
      good.replace("/openai/codex", "/openai/nested/codex"),
      good.replace("/openai/codex", "/openai//codex"),
      good.replace("/openai/codex", "/openai/../codex"),
      good.replace("/openai/codex", "/openai/%2e%2e/codex"),
      good.replace("/openai/codex", "/openai/%252e%252e/codex"),
      good.replace("/openai/codex", "/openai%2fcodex"),
      good.replace("/openai/codex", "/openai\\codex"),
  ...
  fn exact_graphql_documents_cannot_be_replaced_appended_or_batched() {
    let query = "query($cursor:String) { viewer { repositories(first:100,after:$cursor) { pageInfo { hasNextPage endCursor } } } }";
    let selected: Scope = serde_json::from_value(json!({
      "origin":"https://api.github.com", "method":"POST", "path":"/graphql",
    ...
    for doc in [
      "mutation { deleteRepository(input:{repositoryId:\"x\"}) { clientMutationId } }".into(),
  ...
  fn http_requests_require_bounded_immutable_descriptors() {
    let mut request =
      request("https://api.github.com/repos/openai/codex/actions/runs?status=in_progress");
