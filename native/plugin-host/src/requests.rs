...
pub struct Broker {
  ...
  // Three independent effect slots and twelve bounded HTTP slots. The latter
  // supports the original GitHub helper's maximum parallel repository scans.
        ...
        if !matches!(result, Ok(None)) {
          ...
          approval.with_request(Kind::Exec, |_, _| {
            ...
            match result.and_then(|output| crate::exec::response(output.unwrap())) {
              ...
              Err(_) => {
                let _ = reply(&channel, Status::Failed);
              }
            }
            Ok(())
          })?;
    ...
    while index < self.clients.len() {
      ...
      let Some(kind) = request.kind() else {
        if let Request::PanelState(state) = request {
          ...
          let _ = reply(&client, Status::Delivered);
        }
        continue;
      };
      ...
      let result = approval.with_request(kind, |id, grants| {
        ...
        if let Request::Settings(context) = &request {
          ...
          grants.settings.check_write(&settings)?;
        }
        request
...
pub fn report_panel_state(serial: &str, open: &str) -> io::Result<()> {
  let state = Control::PanelState {
    serial: serial
      .parse()
      .map_err(|_| invalid("invalid panel serial"))?,
    open: open.parse().map_err(|_| invalid("invalid panel state"))?,
  };
...
pub fn open_url(mode: &str, url: &str) -> io::Result<()> {
  let mode = match mode {
    ...
    _ => return Err(invalid("unknown browser mode")),
  };
  let request = OpenUrl {
    version: 1,
    mode,
  ...
  request.validate()?;
  let channel = Channel::connect(Path::new("/run/plugin/open-url"))?;
