...
mod tests {
  ...
  fn plugin_data_and_assets_resolve_independently() {
    let dirs = [
      PluginDir::assets("/run/user/1000/omarchy/plugin/slcode777.omagotchi".to_owned()),
      PluginDir::data("/home/me/.local/state/omarchy/plugins/slcode777.omagotchi".to_owned()),
    ];
    // Each token resolves to its own directory.
    assert_eq!(
      resolve_plugin_path("$OMARCHY_PLUGIN_DATA/save.json", &dirs).unwrap(),
      "/home/me/.local/state/omarchy/plugins/slcode777.omagotchi/save.json"
    );
    assert_eq!(
      resolve_plugin_path("$OMARCHY_PLUGIN_PATH/sounds/a.wav", &dirs).unwrap(),
      "/run/user/1000/omarchy/plugin/slcode777.omagotchi/sounds/a.wav"
    );
  ...
  fn check_with_resolves_plugin_path_symmetrically_and_rejects_escape() {
    ...
    let root = "/run/user/1000/omarchy/plugin/slcode777.omagotchi";
    let dirs = [PluginDir::assets(root.to_owned())];
    ...
    assert!(
      policy
        .check_with(
          &dirs,
          &selected(&["play"]),
          &args(&["/run/user/1000/omarchy/plugin/slcode777.omagotchi/sounds/../x"])
        )
        .is_err()
    );
    assert!(
