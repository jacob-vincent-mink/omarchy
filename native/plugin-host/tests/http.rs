...
fn http_worker_child() {
  ...
  if mode == "denied" {
    ...
    .send(&channel)
    .unwrap();
    assert!(omarchy_plugin_host::http::receive(&channel).is_err());
  } else if mode == "allowed" {
    let (head, body) =
      omarchy_plugin_host::http::receive(&send(&origin, "large", "/large")).unwrap();
    ...
    for (scope, path) in [("large", "/other"), ("missing", "/large")] {
      assert!(omarchy_plugin_host::http::receive(&send(&origin, scope, path)).is_err());
    }
    let channel = connect(Path::new("/run/plugin/http"));
    channel
    ...
    let (head, body) = omarchy_plugin_host::http::receive(&channel).unwrap();
    assert_eq!(head.status, 200);
    assert_eq!(body.len(), 131072);
    let (head, body) =
      omarchy_plugin_host::http::receive(&send(&origin, "redirect", "/redirect")).unwrap();
    assert_eq!(head.status, 302);
    assert_eq!(body, b"redirect");
    assert!(head.headers["location"].ends_with("/other"));
    assert!(omarchy_plugin_host::http::receive(&send(&origin, "oversize", "/oversize")).is_err());
    let requests: Vec<_> = (0..12).map(|_| send(&origin, "slow", "/slow")).collect();
    assert!(omarchy_plugin_host::http::receive(&send(&origin, "slow", "/slow")).is_err());
    for channel in requests {
      assert_eq!(
        omarchy_plugin_host::http::receive(&channel)
          .unwrap()
          .0
          .status,
        200
...
fn admitted_http_is_scoped_bounded_and_revocable_in_a_real_worker() {
  ...
  for mode in ["denied", "allowed", "timeout", "revoke"] {
    ...
    let scopes: serde_json::Map<String, serde_json::Value> = ["large", "slow", "redirect", "oversize"].into_iter()
    ...
    fs::write(source.join("manifest.json"), serde_json::to_vec(&serde_json::json!({
      ...
      "sandbox":{"version":1, "entryPoint":"worker.qml", "requests":{"http":scopes,"notifications":true}}
    })).unwrap()).unwrap();
    fs::write(
