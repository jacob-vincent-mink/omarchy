...
fn private_layers_preserve_focus_and_constrain_committed_popups() {
  ...
  let session = Session::start(
    state,
    "test.surfaces".into(),
    env!("CARGO_BIN_EXE_omarchy-plugin-host").into(),
    Viewport {
      width: 400,
      height: 300,
      scale: 1,
      ...
      .unwrap_or_else(|error| Some(Update::Failed(error.to_string())))
    {
      ...
      Some(Update::Presentation(Event::Frame { serial, slot, .. })) => {
          ...
          && pixel(200, 190) == [0x30, 0x40, 0x50]
        {
          ...
          let x = orange.unwrap();
          if pet_first.is_some_and(|first: usize| first.abs_diff(x) >= 8) {
            ...
            key(26);
            phase = 1;
          } else if pet_first.is_none() {
            pet_first = Some(x);
          }
        } else if phase == 1 && count([0xff, 0xee, 0x44]) == 400 {
          pet_x = orange.unwrap() + 16;
          click(pet_x as i32, 35);
          key(27);
          phase = 2;
        } else if phase == 2 && count([0xee, 0x55, 0x99]) > 300 && count([0xee, 0xee, 0xff]) == 400
        {
          ...
          assert_eq!(
            pixel(pet_x, 35),
            [0x20, 0xbf, 0xa5],
            "demoted pet still above top bar"
          );
          click(pet_x as i32, 35);
          phase = 3;
        } else if phase == 3 && pixel(pet_x, 35) == [0x44, 0xee, 0x22] {
          click(358, 24);
          phase = 4;
