...
- Ward is the security boundary for third-party plugins distributed through the Omarchy plugin registry, not for Omarchy's first-party plugins. Preserve an explicit trusted in-process installation path for users' own plugins. A downloaded manifest cannot grant itself trusted status. Keep concrete plugin compatibility fixtures outside `native/ward`; Ward's tests use synthetic fixtures and generic resource contracts.
